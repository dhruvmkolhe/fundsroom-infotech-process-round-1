...
A production-ready full-stack Enterprise Resource Planning (ERP) and Customer Relationship Management (CRM) platform designed for wholesale and distribution enterprises. Built with Node.js, Express, TypeScript, SQLite, and React.
...
### 2. Start Backend REST API & Frontend SPA Concurrently
```bash
npm run dev
```

- **Frontend Application UI**: [http://localhost:5173](http://localhost:5173)
- **Backend REST API Server**: [http://localhost:5000/api](http://localhost:5000/api)

### 3. Demo Credentials (Role-Based Access)

| Role | Email | Password | Privileges & Access Scope |
|---|---|---|---|
| **Admin** | `admin@distroerp.com` | `admin123` | Master control across Customers, Inventory, Sales Challans, and Logs |
| **Sales** | `sales@distroerp.com` | `sales123` | Customer CRM management & Sales Challans creation/confirmation |
| **Warehouse** | `warehouse@distroerp.com` | `warehouse123` | Product catalog management & Stock IN/OUT adjustment logs |
| **Accounts** | `accounts@distroerp.com` | `accounts123` | Financial audit, revenue reports, and challan details view |
