...
router.get('/:id', authenticate, async (req, res) => {
  try {
    const db = await getDb();
    const p = await db.get<any>('SELECT * FROM products WHERE id = ?', [req.params.id]);
    if (!p) {
...
router.post('/', authenticate, requireRole(['Admin', 'Warehouse']), async (req: AuthRequest, res) => {
  try {
    ...
    const countRes = await db.get<{ count: number }>('SELECT COUNT(*) as count FROM products');
    const newId = `P${String((countRes?.count || 0) + 1).padStart(3, '0')}`;
    const now = new Date().toISOString();

    await db.run(
      `INSERT INTO products (id, name, sku, category, unit_price, current_stock, min_stock_alert, warehouse_location, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        newId,
        name.trim(),
    ...
    const created = await db.get<any>('SELECT * FROM products WHERE id = ?', [newId]);
...
router.put('/:id', authenticate, requireRole(['Admin', 'Warehouse']), async (req: AuthRequest, res) => {
  try {
    const { name, sku, category, unitPrice, minStockAlert, warehouseLocation } = req.body;

    const db = await getDb();
    const existing = await db.get<any>('SELECT * FROM products WHERE id = ?', [req.params.id]);
    if (!existing) {
      return res.status(404).json({ error: 'Product not found.' });
    }
    ...
    const updated = await db.get<any>('SELECT * FROM products WHERE id = ?', [req.params.id]);
    return res.json({
      id: updated.id,
      name: updated.name,
      sku: updated.sku,
...
router.post('/:id/adjust-stock', authenticate, requireRole(['Admin', 'Warehouse']), async (req: AuthRequest, res) => {
  try {
