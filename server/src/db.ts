...
async function seedData(db: Database) {
  ...
  const custCount = await db.get<{ count: number }>('SELECT COUNT(*) as count FROM customers');
  if (custCount && custCount.count === 0) {
    await db.run(
      `INSERT INTO customers (id, name, mobile, email, business_name, gst, type, address, status, follow_up_date, created_at) VALUES
      (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?),
      (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?),
      (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?),
      (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  ...
  const prodCount = await db.get<{ count: number }>('SELECT COUNT(*) as count FROM products');
  if (prodCount && prodCount.count === 0) {
    const now = new Date().toISOString();
    await db.run(
      `INSERT INTO products (id, name, sku, category, unit_price, current_stock, min_stock_alert, warehouse_location, created_at) VALUES
      (?, ?, ?, ?, ?, ?, ?, ?, ?),
      (?, ?, ?, ?, ?, ?, ?, ?, ?),
      (?, ?, ?, ?, ?, ?, ?, ?, ?),
      (?, ?, ?, ?, ?, ?, ?, ?, ?),
  ...
  const smCount = await db.get<{ count: number }>('SELECT COUNT(*) as count FROM stock_movements');
  if (smCount && smCount.count === 0) {
    await db.run(
      `INSERT INTO stock_movements (id, product_id, product_name, product_sku, qty_changed, type, reason, created_by, timestamp) VALUES
      (?, ?, ?, ?, ?, ?, ?, ?, ?),
      (?, ?, ?, ?, ?, ?, ?, ?, ?),
      (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
  ...
  const chCount = await db.get<{ count: number }>('SELECT COUNT(*) as count FROM challans');
  if (chCount && chCount.count === 0) {
    await db.run(
      `INSERT INTO challans (id, number, customer_id, customer_name, customer_business, total_qty, total_amount, status, created_by, created_date) VALUES
      (?, ?, ?, ?, ?, ?, ?, ?, ?, ?),
      (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
    ...
    await db.run(
      `INSERT INTO challan_items (id, challan_id, product_id, product_name, sku, unit_price, qty, subtotal) VALUES
      (?, ?, ?, ?, ?, ?, ?, ?),
      (?, ?, ?, ?, ?, ?, ?, ?),
      (?, ?, ?, ?, ?, ?, ?, ?)`,
      [
