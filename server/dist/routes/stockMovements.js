...
router.get('/', authenticate, async (req, res) => {
    try {
        ...
        const countRes = await db.get(`SELECT COUNT(*) as total FROM stock_movements ${whereSql}`, params);
        const total = countRes?.total || 0;
        const movements = await db.all(`SELECT * FROM stock_movements ${whereSql} ORDER BY id DESC LIMIT ? OFFSET ?`, [...params, limitNum, offset]);
