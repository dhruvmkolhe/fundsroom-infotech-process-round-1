...
router.get('/', authenticate, async (req, res) => {
    try {
        ...
        const countRes = await db.get(`SELECT COUNT(*) as total FROM customers ${whereSql}`, params);
        const total = countRes?.total || 0;
        const customers = await db.all(`SELECT * FROM customers ${whereSql} ORDER BY created_at DESC LIMIT ? OFFSET ?`, [...params, limitNum, offset]);
        // Format response customers
        const formattedCustomers = await Promise.all(customers.map(async (c) => {
            const notes = await db.all(`SELECT text, by_name as by, created_at as at FROM customer_notes WHERE customer_id = ? ORDER BY created_at DESC`, [c.id]);
            return {
                id: c.id,
                name: c.name,
                mobile: c.mobile,
...
router.post('/', authenticate, requireRole(['Admin', 'Sales']), async (req, res) => {
    try {
        ...
        const db = await getDb();
        const countRes = await db.get('SELECT COUNT(*) as count FROM customers');
        const newId = `C${String((countRes?.count || 0) + 1).padStart(3, '0')}`;
        const now = new Date().toISOString();
        await db.run(`INSERT INTO customers (id, name, mobile, email, business_name, gst, type, address, status, follow_up_date, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
            newId,
            name.trim(),
            mobile.trim(),
        ...
        if (notesText && notesText.trim()) {
            ...
            await db.run(`INSERT INTO customer_notes (id, customer_id, text, by_name, created_at) VALUES (?, ?, ?, ?, ?)`, [noteId, newId, notesText.trim(), req.user?.name || 'System', formattedDate]);
        }
        const created = await db.get('SELECT * FROM customers WHERE id = ?', [newId]);
        const notes = await db.all(`SELECT text, by_name as by, created_at as at FROM customer_notes WHERE customer_id = ?`, [newId]);
...
router.put('/:id', authenticate, requireRole(['Admin', 'Sales']), async (req, res) => {
    try {
        ...
        await db.run(`UPDATE customers SET
       ...
       WHERE id = ?`, [
        ...
        const notes = await db.all(`SELECT text, by_name as by, created_at as at FROM customer_notes WHERE customer_id = ? ORDER BY created_at DESC`, [req.params.id]);
        return res.json({
            id: updated.id,
            name: updated.name,
...
router.post('/:id/notes', authenticate, async (req, res) => {
    try {
        ...
        const noteId = `CN_${Date.now()}`;
        const formattedDate = new Date().toLocaleString('en-IN', { dateStyle: 'medium', timeStyle: 'short' });
        await db.run(`INSERT INTO customer_notes (id, customer_id, text, by_name, created_at) VALUES (?, ?, ?, ?, ?)`, [noteId, req.params.id, text.trim(), req.user?.name || 'User', formattedDate]);
        const notes = await db.all(`SELECT text, by_name as by, created_at as at FROM customer_notes WHERE customer_id = ? ORDER BY created_at DESC`, [req.params.id]);
