...
async function getChallanFormatted(db, id) {
    const c = await db.get('SELECT * FROM challans WHERE id = ?', [id]);
    if (!c)
        return null;
    const items = await db.all(`SELECT product_id as productId, product_name as productName, sku, unit_price as unitPrice, qty, subtotal
     FROM challan_items WHERE challan_id = ?`, [id]);
    return {
        id: c.id,
        number: c.number,
...
router.get('/', authenticate, async (req, res) => {
    try {
        ...
        const countRes = await db.get(`SELECT COUNT(*) as total FROM challans ${whereSql}`, params);
        const total = countRes?.total || 0;
        const rows = await db.all(`SELECT id FROM challans ${whereSql} ORDER BY created_date DESC, id DESC LIMIT ? OFFSET ?`, [...params, limitNum, offset]);
...
router.post('/', authenticate, requireRole(['Admin', 'Sales']), async (req, res) => {
    try {
        ...
        const countRes = await db.get('SELECT COUNT(*) as count FROM challans');
        ...
        // Insert Challan
        await db.run(`INSERT INTO challans (id, number, customer_id, customer_name, customer_business, total_qty, total_amount, status, created_by, created_date)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
            newId,
            challanNumber,
            cust.id,
        ...
        for (let i = 0; i < validatedItems.length; i++) {
            ...
            await db.run(`INSERT INTO challan_items (id, challan_id, product_id, product_name, sku, unit_price, qty, subtotal)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [itemId, newId, item.productId, item.productName, item.sku, item.unitPrice, item.qty, item.subtotal]);
            if (status === 'Confirmed') {
                // Stock reduction
                await db.run(`UPDATE products SET current_stock = current_stock - ? WHERE id = ?`, [item.qty, item.productId]);
...
router.post('/:id/confirm', authenticate, requireRole(['Admin', 'Sales']), async (req, res) => {
    try {
        const db = await getDb();
        const challan = await db.get('SELECT * FROM challans WHERE id = ?', [req.params.id]);
        if (!challan) {
        ...
        const items = await db.all(`SELECT * FROM challan_items WHERE challan_id = ?`, [req.params.id]);
        // Verify stock for all items BEFORE performing changes
        for (const item of items) {
            const p = await db.get('SELECT current_stock, name FROM products WHERE id = ?', [item.product_id]);
            if (!p) {
        ...
        for (let i = 0; i < items.length; i++) {
            const item = items[i];
            await db.run(`UPDATE products SET current_stock = current_stock - ? WHERE id = ?`, [item.qty, item.product_id]);
            const smId = `SM_${Date.now()}_${i}`;
            await db.run(`INSERT INTO stock_movements (id, product_id, product_name, product_sku, qty_changed, type, reason, created_by, timestamp)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
                smId,
...
router.post('/:id/cancel', authenticate, requireRole(['Admin', 'Sales']), async (req, res) => {
    try {
        const db = await getDb();
        const challan = await db.get('SELECT * FROM challans WHERE id = ?', [req.params.id]);
        if (!challan) {
